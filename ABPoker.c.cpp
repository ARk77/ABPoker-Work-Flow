// Poker.c
// From the Absolute Beginner's Guide to C

/* This program plays draw poker. Users can play as often as they want,
betting between 1 and 5. They are dealt 5 cards and then
get to choose which cards to keep, and which cards to replace. The new
hand is then reviewed and the user's payout is set based on the
value of the hand. The user's new bankroll is displayed as they
are given the option to continue. */
...
// 2 constants defined for determining whether hands are flushes or straights
...
main()
{
	...
	int suitsinHand[4]; // Used for evaluating the final
	int winnings;
	time_t t;
	...
	/* Outside the do...while loop. Greeting will be displayed once
	and then the rest of main will execute until the Player decides
	to stop playing. */
	...
	// Loop runs each time the user plays a hand of poker
	...
	do {
		...
		/* These two arrays are used to figure out the values
		...
		winning = analyzeHand(ranksinHand, suitsinHand);
		printf("You won %d!\n", bet*winnings);
...
// Quick greeting and information on on different winning hands
...
void getFirstHand(int cardRank[], in cardSuit[])
{
	int i, j;
	int cardDup;
	...
	for (i = 0; i < 5; i++)
	{
		...
		else if (toupper(ans) == 'N')
		{
			...
			do {
				...
				// First check your new card against the 6 original
